...
export default function OfferDetailsProgress({ allocations, isSoldOut, progressColors = defaultProgressColors }) {
    ...
    const { base_percentage, base_rounding, res_percentage, res_rounding, guaranteed_percentage, guaranteed_rounding } =
        useOfferDetailsProgress(allocations, isSoldOut);

    useEffect(() => VanillaTilt.init(tilt.current, { scale: 1.05, speed: 1000, max: 5 }), []);

    return (
        <div
            className={cn("relative h-[50px] w-full flex flex-row items-center rounded-xl select-none", {
                "overflow-hidden": base_percentage >= 95,
            })}
            ref={tilt}
        >
