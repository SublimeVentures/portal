...
async function reserveSpot(user, req) {
    ...
    const nftId = id.replace(/[^0-9]/g, '');
    const passedId = nftId ? nftId : 0
    const isBooked = await bookAllocation(offerId, isSeparatePool, TOTAL_ALLOCATION, address, hash, AMOUNT, ACL, passedId)
