...
export const blockchainPrerequisite = async (params) => {
    ...
    console.log("TAK_OFFER_VALID2", signature, signature?.expiry && signature.expiry > moment.utc().unix() && !offerDetails.isSell, offerDetails.isSell)
    ...
    if (
        ...
        const transaction = await getSignature(offerDetails.offerId, requiredNetwork, offerDetails.otcId, offerDetails.dealId, account)
    ...
    const contentQuery = () => {
        return (
            <div className="flex flex-col flex-1">
                <div>{ offerDetails?.isSell ? "Are you sure you want to buy allocation from this SELL offer?" : "Are you sure you want to sell your allocation to this BUY offer?" }</div>
                <div className="grid gap-1 grid-cols-2 my-10 mb-0">
                    ...
                    <div className={"text-right flex flex-row justify-end"}><DynamicIcon
                        name={NETWORKS[chainDesired?.id]} style={ButtonIconSize.hero4}/>
