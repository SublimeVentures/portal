...
export default function TokenomicsCitCap({}) {
    ...
    const processEnvs = () => {
        if(data?.length>0) {
            ...
            const rewardRate_final = Object.keys(rewardRate).map(key => ({ label: `${key} point${key > 1 ? "s" : ""}`, key: Number(rewardRate[key]), value: key }));
            setRewardRateLabel(rewardRate_final)

            const alloTrait_final = Object.keys(alloTrait).map(key => ({ label: `${key}`, key: Number(alloTrait[key]), value: key }));
            setAlloTraitLabel(alloTrait_final)

            const stakeLength_final = Object.keys(stakeLength).map(key => ({ label: `${key} days`, key: Number(stakeLength[key]), value: key }));
            setStakeLengthLabel(stakeLength_final)

            const allocationBase_final = Object.keys(allocationBase).map(key => ({ label: `${CITIZENS_NAME[key]}`, key: allocationBase[key], value: CITIZENS[key] }));
            ...
            <div className="flex flex-col p-10 sm:p-20 w-full  font-accent blurred glareBg bg-black">
                ...
                <div className=" flex flex-1 flex-col gap-5 custom:flex-row">
                    <div className=" flex flex-1 flex-col mt-[1px]">
                        ...
                        {citizenType?.value < CITIZENS.S2 ?   <div className={'pt-10'}>
                ...
                <div className={"mt-10 text-center text-xl"}>
                    <span className={""}>Maximum allocation </span><span className={"font-bold text-gold"}>${allocationTotal || 0}</span>
                </div>
