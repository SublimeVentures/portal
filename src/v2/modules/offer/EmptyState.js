...
export default function EmptyState({ heading, description, cta = null, secondaryCta = null, className = "" }) {
    return (
        <div className={cn("p-4 h-full flex flex-col items-center justify-center bg-white/[.04] gap-2", className)}>
            <h3 className="tracking-tight text-base md:text-lg font-medium text-foreground">{heading}</h3>
            <p className="max-w-2xl text-xs font-light text-foreground/70 text-center lg:text-sm">{description}</p>
