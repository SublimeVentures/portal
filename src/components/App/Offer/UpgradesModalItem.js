...
export default function UpgradesModalItem({itemType, selected, setSelectedUpgrade, isRightPhase, owned, used, image, name, description}) {
    ...
    return (
        <div className={"relative"}>
            <div className={`flex flex-row upgrade ${isBased ? "rounded-xl" : ""} ${selected === itemType ? 'active' : ''} ${!owned ? 'disabled' :''}`} onClick={()=> { if(owned>0) setSelectedUpgrade(itemType) }}>
            {/*<div className={`flex flex-row upgrade ${isBased ? "rounded-xl" : ""} ${selected === itemType ? 'active' : ''} ${!isRightPhase ? 'blur disabled' :'cursor-pointer'} ${!owned ? 'disabled' :''}`} onClick={()=> { if(isRightPhase && owned>0) setSelectedUpgrade(itemType) }}>*/}
                ...
                <div className={"pl-5 py-2 flex flex-col"}>
                    ...
                    <div className={"text-sm"}>
                        <div className={"detailRow mt-auto"}><p>Owned</p><hr className={"spacer"}/><p>{owned ? owned : 0}</p></div>
                    </div>
                </div>
            </div>
