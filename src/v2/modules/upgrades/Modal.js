...
export const Image = forwardRef(({ className, ...props }, ref) => {
    return (
        <div className="h-40 md:h-auto -mx-4 -mt-8 md:mr-0 md:-ml-13 md:-my-8 relative md:w-1/2 grow-0 shrink-0">
            <NextImage fill {...props} ref={ref} className={cn("rounded-t md:rounded-l object-cover", className)} />
            <Logo className="hidden sm:block absolute bottom-3 left-3 md:bottom-6 md:left-6 size-9 md:size-13" />
        </div>
    );
});
...
export const Title = forwardRef(({ className, children, ...props }, ref) => {
    return (
        <h1 {...props} ref={ref} className={cn("text-xl md:text-2xl mb-2 md:mb-6 font-heading", className)}>
            {children}
        </h1>
    );
});
