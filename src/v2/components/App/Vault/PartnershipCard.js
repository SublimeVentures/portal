...
const PartnershipCard = ({ title, description, partners = [], isLoading, isError }) => {
    ...
    return (
        <Card variant="static" className="p-14 pt-20 flex flex-col items-center gap-3 mt-20 grow">
            <ul className="absolute top-0 translate-y-[-50%] flex -space-x-4">
                {partners.map((partner) => (
                    <li key={partner.id}>
                        <AvatarRoot
                            size="large"
                            className={cn("bg-black shadow-[4px_3px_22px] shadow-black", partner.styles)}
                        >
            ...
            <CardDescription className="text-sm font-light text-center text-foreground/[.47]">
                {description}
            </CardDescription>
        </Card>
    );
