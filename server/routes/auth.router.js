...
router.post('/login', async (req, res) => {
    if (!req.body?.message || !req.body?.signature || !req.body?.tenant || !req.body?.partner) return res.status(400).json({});
    try {
...
router.delete('/login', async (req, res) => {
    ...
    try {
        ...
        const auth = await axios.delete(`${process.env.AUTHER}/auth/login`, {data: {token}});
        const result = auth.data
