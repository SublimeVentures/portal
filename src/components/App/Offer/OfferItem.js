...
export default function OfferItem({offer, ACL, cdn}) {
    ...
    return (
        <div
            ...
            ref={imageTilt}>
            <Link href={`${PAGE.Opportunities}/${slug}`} className={`flex flex-1 flex-col bg-navy-accent ${isBased ? "rounded-xl" : ""}`}>
                <div className="bg-center relative min-h-[300px]">
                    <div className={'image-container min-h-[300px]'}>
                        <Image src={`${cdn}/research/${slug}/bg.jpg`} fill className={`imageOfferList ${isBased ? "rounded-tl-xl rounded-tr-xl" : ""}  `} alt={slug} sizes="(max-width: 768px) 100vw"/>
                    </div>
                </div>
