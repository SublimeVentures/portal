...
export default function DetailsSidebar({model, setter, claimModalProps, userId}) {
        ...
        return (
            <div className={`flex flex-1 flex-col`}>
                <div className={`${isBased ? "" : "font-accent"} text-md pt-2`}>
                    ...
                    <div className={"detailRow mb-2"}><p>Participated</p><hr className={"spacer"}/><p className={"font-mono"}>{participated}</p></div>
                    <div className={"detailRow "}><p>Next unlock</p><hr className={"spacer"}/><p className={"font-mono"}>{nextUnlock !== 0 ? nextUnlock : "TBA"}</p></div>
                    <div className={"detailRow "}><p>Allocation snapshot</p><hr className={"spacer"}/><p className={"font-mono"}>{nextSnapshot !== 0 ? nextSnapshot : "TBA"}</p></div>
                    <div className={"detailRow "}><p>Claim date</p><hr className={"spacer"}/><p className={"font-mono"}>{nextClaim !== 0 ? nextClaim : "TBA"}</p></div>
                </div>
                ...
                <div className={`mt-auto ${isBased ? "fullWidth" : " w-full fullBtn"}`}>
                    <UniButton
                        ...
                        state={"danger"}
                        text={"Payout"}
                        handler={()=>{openPayoutClaim()}}/>
                </div>
                {isNextPayout && <ClaimPayoutModal model={payoutClaimOpen} setter={()=> closeClaimPayoutModal()} props={payoutClaimProps}/>}
            </div>
        )
    }
    ...
    return (
        <GenericRightModal  isOpen={model} closeModal={()=> closeModal()} title={title()} content={content()} persistent={false}/>)
}
