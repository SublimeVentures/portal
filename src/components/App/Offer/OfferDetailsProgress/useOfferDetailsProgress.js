...
export default function useOfferDetailsProgress(allocations, isSoldOut) {
    ...
    const base_percentage_raw = (amt_filled / amt_total) * 100 || 0;
    const res_percentage_raw = (amt_res / amt_total) * 100 || 0;
    const guaranteed_percentage_raw = (amt_guaranteed / amt_total) * 100 || 0;
    ...
    const remaining_for_res = (isSettled || isFullfield) ? 0 : 100 - filled_base_percentage;
    const res_adjusted_percentage = Math.min(res_percentage_raw, remaining_for_res);

    const remaining_for_guaranteed = (isSettled || isFullfield) ? 0 : remaining_for_res - res_adjusted_percentage;
    const guaranteed_adjusted_percentage = Math.min(guaranteed_percentage_raw, remaining_for_guaranteed);

    const base_percentage = (isSettled || isFullfield) ? 100 : Math.round(filled_base_percentage);
