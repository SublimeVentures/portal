        ...
        <Card variant="none" className="flex flex-col gap-8 h-full w-full bg-settings-gradient md:py-6 md:px-12">
            <div className="flex justify-between items-center">
                ...
                <div className="hidden md:block">
                    <AddWalletModal isOpen={isAddWalletModalOpen} setIsOpen={setIsAddWalletModalOpen} disabled={isMaxWallets || isLoading} />
                </div>
            </div>
            ...
            <div>
                {isLoading ? (
                    ...
                    <ul>
                        {sortedWallets?.length ? sortedWallets.map(({ wallet, isStaking, isDelegate, isAirdrop, chainId }) => {
                            const address = isDesktop ? wallet : shortenAddress(wallet, 10);

                            return (
                                <li key={wallet.wallet} className="mb-4 py-4 px-8 flex justify-between items-center bg-foreground/[.02] rounded">
                                    <div className="flex flex-col gap-4">
                                        <div className="flex items-center gap-2">
                                            <h4 className="text-foreground">{address}</h4>
                                            {isAirdrop && <Badge variant="warning">{AIRDROP_NETWORKS[chainId]}</Badge>}
                                        </div>
                                        <div className="flex flex-wrap items-center gap-8">
                                            <CheckboxField checked={isAirdrop} className="cursor-default pointer-events-none">Airdrop</CheckboxField>
                                            <CheckboxField checked={isStaking} className="cursor-default pointer-events-none">Staking</CheckboxField>
                                            <CheckboxField checked={isDelegate} className="cursor-default pointer-events-none">Delegated wallet</CheckboxField>
                                        </div>
                                    </div>

                                    {isAirdrop && <RemoveWalletModal wallet={wallet} isOpen={isRemoveWalletModalOpen} setIsOpen={setIsRemoveWalletModalOpen} />}
                                </li>
                            );
                        }) : null}
                    </ul>
