...
export default function RestoreModal({ open, investmentAmount, allocationOld, timeOld, onOpenChange, bookingRestore, bookingCreateNew }) {
            ...
            <DialogContent className="max-w-[1000px]">
                ...
                <div className="my-2 px-4 py-4 flex flex-col gap-x-4 bg-white/[.04] md:px-8 md:flex-row">
                    <DefinitionList className="grid-cols-[45%,1fr] grid-rows-6 w-full md:grid-cols-[72px,auto,auto,auto,auto] md:grid-rows-1">
                        ...
                        <Definition termClassName="md:order-4" descClassName="md:order-4" term="Amount">{allocationOldLocal}</Definition>
                        <Definition termClassName="md:order-3" descClassName="md:order-3" term="Date">{timeOldFormatted}</Definition>
                    ...
                    <Button onClick={bookingRestore} className='mt-4 w-full md:mt-0 md:w-max md:h-max md:self-center'>Restore</Button>
                </div>

                <DialogFooter className="md:flex-row items-center justify-between">
                    <p className="order-2 text-xs text-foreground/50 md:order-1 md:text-sm">Reservation will be automaticaly restored.</p>
