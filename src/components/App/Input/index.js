    ...
    useEffect(() => {
        if (type === "number") {
            if (min && input < min) {
                ...
                return setIsError({
                    state: true,
                    msg: `Minimum amount: ${extraCurrency ? "" : "$"}${min.toLocaleString()}${extraCurrency ? ` ${extraCurrency}` : ""}`,
                });
            } else if (max && input > max) {
                setStatus(true);
                return setIsError({
                    state: true,
                    msg: `Maximum amount: ${extraCurrency ? "" : "$"}${max.toLocaleString()}${extraCurrency ? ` ${extraCurrency}` : ""}`,
                });
            } else {
                ...
                if (max) {
                    return setIsError({
                        state: false,
                        msg: `Maximum amount: $${max.toLocaleString()}`,
                    });
                } else return setIsError({ state: false });
            }
        } else if (type === "wallet") {
            setStatus(!isAddress(input));
            return setIsError({
                state: !isAddress(input),
                msg: `Provided address is not valid`,
            });
        }
    }, [input]);
    ...
    return (
        ...
        >
            ...
            {after && (
                <div
                    className={
                        "after rounded-tr-md rounded-br-md px-5 flex justify-center items-center mt-[1px] mb-[1px]"
                    }
                >
                    {after}
                </div>
