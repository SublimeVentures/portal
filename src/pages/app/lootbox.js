...
export default function AppLootbox({account}) {
    ...
    const {isSuccess: lootboxAmountIsSuccess, data: lootboxAmount} = useQuery({
            queryKey: ["lootboxAmount", {ACL, address}],
            queryFn: fetchLootbox,
            refetchOnMount: false,
            refetchOnWindowFocus: false,
            cacheTime: 0,
        }
    );
    const availableAmount = lootboxAmountIsSuccess ? lootboxAmount.length : 0
        ...
        <>
            ...
            <div className={`flex flex-1 flex-col select-none justify-center items-center gap-y-5 mobile:gap-y-10 mobile:gap-10 loot ${isBased ? "" : "font-accent"}`}>
                ...
                <div className={"flex gap-5 mt-10 sm:mt-0"}>
                    {/*<UniButton type={ButtonTypes.BASE} text={'OPEN'} state={"success"} isDisabled={availableAmount === 0}*/}
                    <UniButton type={ButtonTypes.BASE} text={'OPEN'} state={"success"} isDisabled={true}
                               isPrimary={true}
                               isWide={true}
                               zoom={1.05}
                               size={'text-sm sm'}
