...
export default function UpgradesModalItem({
    ...
    return (
        <div className="relative">
            <div
                className={`flex flex-row upgrade bordered-container ${selected === itemType ? "active" : ""} ${!isRightPhase || (isRightPhase && itemType === PremiumItemsENUM.Guaranteed && used > 0) ? "blur disabled" : "cursor-pointer"} ${!owned ? "disabled" : ""}`}
                onClick={() => {
