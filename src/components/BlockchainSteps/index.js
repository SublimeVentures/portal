...
const BlockchainSteps = ({data}) => {
    ...
    const allowance_current = useGetTokenAllowance(allowance_shouldRun, token, params.account, params.spender, chainId, !steps.allowance)
    const allowance_mustRun = allowance_shouldRun && allowance_current.isFetched && allowance_current.allowance < params.allowance

    const allowance_methodReset = steps.allowance ? getMethod(METHOD.ALLOWANCE, token, {...params, allowance: 0, chainId}) : {method: {stop:true}}
    const allowance_method = steps.allowance ? getMethod(METHOD.ALLOWANCE, token, {...params, chainId}): {method: {stop:true}}
    ...
    console.log("METHOD VALIDATION HIXKFHERYDDDD", {allowance_methodReset, allowance_method })

    const allowance_needReset = allowance_mustRun && allowance_current.allowance > 0 && token?.contract.toLowerCase() === ETH_USDT.toLowerCase() && allowance_methodReset.ok
    ...
    console.log(`BIX :: HIXKFHERYDDDD ALLOWANCE CHANGE - HOOK STATE`, {allowance_set_reset, allowance_set})
    const allowance_isFinished = (!allowance_mustRun && !state.allowance.executing && params.allowance <= allowance_current?.allowance) || (state.allowance.executing && allowance_set?.confirm?.data && params.allowance <= allowance_current?.allowance)
    console.log(`BIX :: HIXKFHERYDDDD ALLOWANCE CHECK - RUN ${allowance_shouldRun}`,allowance_current?.allowance, allowance_isFinished, (!allowance_mustRun && !state.allowance.executing && params.allowance <= allowance_current?.allowance), (state.allowance.executing && allowance_set?.confirm?.data && params.allowance <= allowance_current?.allowance))
    ...
    const stepAllowance = StepsState(STEPS.ALLOWANCE, state.allowance, {...allowance_current, token, params, allowance_isFinished, allowance_set_reset, allowance_set, allowance_isReady, allowance_method_error, allowance_shouldRun })
    const stepPrerequisite = StepsState(STEPS.PREREQUISITE, state.prerequisite, {...prerequisite, token, params, prerequisite_isReady })
    const stepTransaction = StepsState(STEPS.TRANSACTION, state.transaction, {...transaction, token, params, transaction_isFinished, transaction_isReady })
