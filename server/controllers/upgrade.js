...
const {
    ...
    roundAmount,
} = require("../../src/lib/investment");

async function useGuaranteed(offerId, user, transaction) {
    const { userId, partnerId, tenantId } = user;

    const userAppliedUpgrades = await fetchUpgradeUsed(
        userId,
        offerId,
        tenantId,
        transaction,
    );
    const upgradeGuaranteed = userAppliedUpgrades?.find(
        (el) => el.id === PremiumItemsENUM.Guaranteed,
    )?.amount;
    const upgradeIncreased = userAppliedUpgrades?.find(
    ...
    const amount =
        maxAllocation < PremiumItemsParamENUM.Guaranteed
            ? maxAllocation
...
async function useUpgrade(user, req) {
    ...
    try {
        ...
        const isUserOwnsUpgrade = await getStoreItemsOwnedByUser(
            userId,
            tenantId,
            storeId,
            transaction,
        ...
        if (storeId === PremiumItemsENUM.Guaranteed) {
            const saveStatus_upgrade = await useGuaranteed(
                offerId,
                user,
                transaction,
            );
            if (!saveStatus_upgrade.ok) {
                logger.warn(
                    "ERROR :: [useUpgrade] - guaranteed couldnt apply",
                    {
                        saveStatus_upgrade,
                        isUserOwnsUpgrade,
                        user,
