            ...
            <div className={`${isBased ? "rounded-xl" : ""} boxshadow relative offerWrap flex flex-1 max-w-[600px]`}>
                <div className={`${isBased ? "rounded-xl" : ""} relative bg-navy-accent flex flex-1 flex-col `}>
                    <div className="font-bold text-2xl flex items-center glowNormal p-5 ">
                        <div className={`flex flex-1 ${isBased ? "" : "text-app-error font-accent glowRed  font-light text-2xl flex glowNormal"} `}>WALLETS</div>
                    ...
                    <div>
                        <table>
                            ...
                            {wallets.map(el => {
                                return <tr key={el.wallet}
                                           className="hoverTable transition-all duration-300 hover:text-black">

                                    <td className={`text-app-error font-bold text-sm text-right px-5 py-1 sm:text-left sm:px-2 sm:py-4 sm:pl-5 sm:pr-2`}
                                        data-label="WALLET">
                                    ...
                                    <td className="text-sm text-right px-5 py-1 sm:text-center  sm:px-2 sm:py-4 sm:px-2"
                                        data-label="STAKING">{el.isStaking ? <DynamicIcon name={ICONS.SUCCESS} style={ButtonIconSize.hero3center}/> : <>-</>}</td>
                                    <td className="text-sm text-right px-5 py-1 sm:text-center sm:px-2 sm:py-4 sm:px-2"
                                        data-label="DELEGATED">{el.isDelegate ? <DynamicIcon name={ICONS.SUCCESS} style={ButtonIconSize.hero3center}/> : <>-</>}</td>
                                </tr>
                            })}
                            </tbody>
                        </table>
                    ...
                    <div className={"flex flex-row gap-5 ml-auto p-5 mt-auto"}>
                            <UniButton type={ButtonTypes.BASE}
                                       ...
                                       handler={()=> { openWalletAdd() }}
                            />
                        {wallets.length > 1 && <UniButton type={ButtonTypes.BASE}
                                       text={'REMOVE'}
                                       size={'text-sm xs'}
                                       isWide={true}
                                       isLarge={true}
                                       isDisabled={wallets.length === 1}
                                       handler={()=> { openWalletRemove() }}
                            />}
                    </div>
                </div>
            </div>

            <WalletAddModal addProps={addProps} model={walletAdd} setter={async () => {
                setWalletAdd(false)
                updateEnvironmentProps([{path: 'walletGuard', value: true}])
                // await refreshSession()
            }}/>
            <WalletRemoveModal addProps={addProps} model={walletRemove} setter={async () => {
