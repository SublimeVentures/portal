...
async function addUserWallet(user, req) {
    try {
        ...
        const session = await axios.post(`${process.env.AUTHER}/session/addWallet`, {token, signature: req.body.signature });
        return session.data
    } catch (error) {
...
async function removeUserWallet(user, req) {
    try {
        const token = req.cookies[authTokenName]
        const session = await axios.post(`${process.env.AUTHER}/session/removeWallet`, {token, signature: req.body.signature });
        return session.data
    } catch (error) {
...
module.exports = {getUserWallets, addUserWallet, removeUserWallet, refreshStaking}
