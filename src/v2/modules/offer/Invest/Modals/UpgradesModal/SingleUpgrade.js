                ...
                <div className="w-full flex flex-col">
                    <div className="space-y-2">
                        ...
                        <p className="text-sm font-light text-foreground md:text-base">
                            {!isValidPhase ? "Can't use this upgrade during this phase." : isGuaranteedUpgradeUsed ? `${upgrade.name} can be used only once.` : upgrade.description}
                        </p>
                    </div>
                    <dl className={cn("mt-auto px-4 py-2 flex items-center gap-x-6 gap-y-2 rounded md:mt-4", isValidPhase ? "text-accent bg-accent/10" : "text-red-500 bg-red-500/10")}>
                        <dt>Owned</dt>
                        ...
                        {id === PremiumItemsENUM.Increased && isUsed && (
                            <>
                                <dt>Used</dt>
                                <dd style={{ textShadow: isValidPhase ? "0 0 15px green" : "0 0 15px green" }}>{usedCount}</dd>
                            </>
                        )}
                    </dl>
                </div>
