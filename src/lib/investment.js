...
function getUserAllocationMax(
    account,
    offer,
    allocationOffer,
    upgradeIncreasedUsed,
) {
    ...
    switch (account.tenantId) {
        ...
        case TENANT.NeoTokyo: {
            allocationUser_base =
                account.multi * allocationOffer?.alloTotal +
                account.allocationBonus;
            if (allocationUser_base < MIN_ALLOCATION)
                allocationUser_base = MIN_ALLOCATION;
            allocationUser_min = MIN_ALLOCATION;
            break;
        }
        case TENANT.CyberKongz: {
            allocationUser_base =
                offer.alloMax * Math.ceil(account.stakeSize / offer.alloMax);
            allocationUser_min = offer.alloMin;
            break;
        }
    }
    allocationUser_max = getUserAllocationBaseWithIncreased(
        allocationUser_base,
        upgradeIncreasedUsed,
    );
...
function getUserAllocationBaseWithIncreased(
    allocationUser_max_base,
    upgradeIncreasedUsed = 0,
) {
    const increasedAllocation =
        upgradeIncreasedUsed * PremiumItemsParamENUM.Increased;
...
function getAllocationLeft(allocationOffer_left, allocationUser_left) {
    return allocationOffer_left < allocationUser_left
...
function allocationParseFCFS(params) {
    ...
    if (
        allocationUser_guaranteed.total > 0 &&
        allocationUser_guaranteed.left >= allocationUser_min
    ) {
        allocationUser_left = allocationUser_guaranteed.left;
    } else {
        allocationUser_left = getAllocationLeft(
            allocationOffer_left,
...
function allocationUserBuild(params) {
    const { allocationUser_base, allocationUser_max, allocationUser_min } =
    ...
    params.output.allocationUser_guaranteed = getUserAllocationGuaranteed(
        params.upgradesUse?.guaranteedUsed,
    );
...
function userInvestmentState(
    ...
    const allocationUser_guaranteed_rounded = roundAmount(
        allocationUser_guaranteed.left,
    );

    console.log("QUELCO - summary", {
        allocationUser_min,
        allocationUser_max:
            allocationUser_max_rounded < 0 ? 0 : allocationUser_max_rounded,
        allocationUser_left:
            allocationUser_left_rounded < 0 ? 0 : allocationUser_left_rounded,
        allocationUser_guaranteed:
            allocationUser_guaranteed_rounded < 0
                ? 0
                : allocationUser_guaranteed_rounded,
        ...
        allocationOffer_left,
        offer_isProcessing:
            allocationOffer_left - 50 <= 0 &&
            allocationOffer?.alloTotal - allocationOffer?.alloFilled + 50 > 0,
        offer_isSettled:
            allocationOffer_left - 50 <= 0 &&
            allocationOffer?.alloTotal - allocationOffer?.alloFilled - 50 <= 0,
    });
...
function tooltipInvestState(offer, allocationData, investmentAmount) {
    if (allocationData.allocationUser_left === 0) {
        ...
        !allocationData.allocationUser_invested &&
        investmentAmount < allocationData.allocationUser_min
    ) {
        return {
            allocation: false,
...
function buttonInvestState(
    ...
    return {
        text: buttonInvestText(
            allocationOffer,
            allocationData,
            offerPhaseCurrent.button,
        ),
