...
export default function ChangeAddress({ session }) {
    ...
    const { account, environmentCleanup, walletGuard } =
    ...
    const isAddressSupported = Boolean(
        walletGuard &&
            ...
            wallets.find((el) => el === userAddress),
    );
    ...
    const contentWrongWallet = () => {
        return (
            <div className={"flex flex-1 flex-col"}>
                ...
                You can only use wallets approved in the{" "}
                <span className={"contents text-app-success"}>
                    Setting
                </span>{" "}
                page.
                <div className={"my-10"}>
                    ...
                    <div
                        className={`text-xs p-2 bg-slides text-center ${isBased ? "rounded-xl" : ""}`}
                    >
                        {userAddress}
                    </div>
                </div>
    ...
    const content = () =>
        !!userAddress ? contentWrongWallet() : contentWalletDisconnected();
