...
export default function PremiumSummary({ data }) {
    ...
    return (
        ...
        >
            <div
                className={
                    "flex flex-1 flex-col md:flex-row justify-center items-center"
                }
            >
                <div
                    className={`flex flex-1 text-3xl font-bold ${isBased ? "" : "font-accent uppercase font-light"}`}
                >
                    Premium
                </div>
                {guaranteed?.amount > 0 ||
                ...
                mystery?.amount > 0 ? (
                    <div
                        className={`blurred2 px-4 py-2 mt-5 md:mt-0  ${isBased ? "rounded-xl" : ""} `}
                    >
                        {guaranteed?.amount > 0 && (
                            <div className={"detailRow"}>
                                <p>Guaranteed Allocation:</p>
                                <hr
                                    className={"spacer min-w-[20px] opacity-0"}
                                />
                                <p>
                                    {guaranteed?.amount ? guaranteed.amount : 0}
                                </p>
                            </div>
                        )}
                        {increased?.amount > 0 && (
                            <div className={"detailRow"}>
                                ...
                                />
                                <p>
                                    {increased?.amount ? increased.amount : 0}
                                </p>
                            </div>
                        )}
                        {mystery?.amount > 0 && (
